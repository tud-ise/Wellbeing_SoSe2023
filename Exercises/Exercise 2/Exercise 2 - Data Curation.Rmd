...
In the so created tibble, we can see that five observations in the "smartphone" column have no entry (`NA`). For our first evaluation, we are **not interested in which smartphone** a character uses, but **only whether s/he has one or not**. For this we can use dummy variables.
